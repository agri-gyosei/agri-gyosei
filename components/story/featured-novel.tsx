...
function NovelEntry({
  ...
  return (
    <div ref={containerRef} id={id} className="relative w-full">
      ...
      {/* ── Mobile: コンパクト横並びカード ── */}
      <div className="lg:hidden px-6 py-8 border-b border-foreground/10 last:border-b-0">
        <div className="flex items-start gap-5">
          <div className="flex-none w-[88px] relative" style={{ height: '132px' }}>
            <Image
              src={coverSrc}
              alt={coverAlt}
              fill
          ...
          <div className="flex-1 min-w-0">
            <p className="text-gold text-[0.6rem] tracking-[0.3em] uppercase mb-1">{genre}</p>
            <h2 className="font-serif text-ink text-xl font-light leading-tight tracking-wide mb-0.5">{title}</h2>
            <p className="text-foreground/50 text-sm tracking-wide mb-2">{subtitle}</p>
            <p className="text-foreground/60 text-xs leading-relaxed mb-4 line-clamp-2">{description}</p>
            <Link
              href={href}
              className="inline-block border border-[#c9a84c] px-4 py-2 text-[#c9a84c] text-[0.65rem] tracking-[0.25em] uppercase"
            >
              読む
            </Link>
          </div>
      ...
      {/* ── Desktop: フルパララックスレイアウト ── */}
      <div className="hidden lg:block py-48">
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid grid-cols-2 gap-24 items-center">
            {/* Book cover */}
            <motion.div
              style={{ y: imageY }}
              className={`relative aspect-[2/3] w-full max-w-md mx-0 ${reverse ? "order-2" : "order-1"}`}
            >
            ...
            {/* Book details */}
            <motion.div style={{ y: textY }} className={`relative ${reverse ? "order-1" : "order-2"}`}>
              <motion.p
              ...
              <motion.p
                ...
                className="text-foreground/50 text-lg tracking-wide mb-8"
              >
                {subtitle}
              </motion.p>
              <motion.div
              ...
              <motion.p
                ...
                className="text-foreground/70 text-lg leading-relaxed max-w-lg mb-16"
              >
                {description}
              </motion.p>
              <motion.div
